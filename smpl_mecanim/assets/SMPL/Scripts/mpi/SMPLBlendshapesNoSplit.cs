...
namespace SMPL.Scripts.mpi {
	public class SMPLBlendshapesNoSplit : MonoBehaviour {
		...
		void UpdatePoseBlendshapeValues() {
			...
			foreach (Transform bone in bones) {
				...
				int singledIndex = boneIndex * 9 ; 
			
				SetBlendShapeWeights(rot3X3, singledIndex);
		...
		void SetBlendShapeWeights(float[] rot3X3, int singledIndex) {
			...
			for (int indexIndRotationMatrix = 0; indexIndRotationMatrix < rot3X3.Length; indexIndRotationMatrix++) {
				...
				int index = singledIndex * 9 + indexIndRotationMatrix;

				if (OptimizePoseBlends && !poseBlendshapesToSet.Contains(index)) continue;

				int positiveBlendshapeIndex = NumberOfShapeParameters + singledIndex + (indexIndRotationMatrix);
				skinnedMeshRenderer.SetBlendShapeWeight(positiveBlendshapeIndex, (theta * 50f + 50f));
			}
		}
		...
		public void SetShapeBlendValues()
		{
			for (int i=0; i < NumberOfShapeParameters; i++)
			{
				...
				int index = i;
				skinnedMeshRenderer.SetBlendShapeWeight(index, (beta * 50.0f + 50f)); // map [-1, 1] space to [0, 100]
			}
